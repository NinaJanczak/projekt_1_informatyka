...
def XYZ2BLH(plik_txt,a,e2):
    ...
    for i in dane_wej:
        ...
        B = B * 180 / pi
        L = L * 180 / pi
        dane_wyj.append([B,L,H])
    return (dane_wyj)

def BLH2XYZ(plik_txt,a,e2):
    dane_wej = Odczyt_pliku(plik_txt)
    dane_wyj = []
    for i in dane_wej:
        B,L,H = i
        ...
        Z = (N * (1 - e2) + H) * np.sin(B)
        dane_wyj.append([X,Y,Z])
    return(dane_wyj)

def XYZ2NEU(plik_txt,a,e2,):
    dane_wej = Odczyt_pliku(plik_txt)
    dane_wyj = []
    for i in dane_wej:
        X,Y,Z,s,alfa,z = i
        ...
        dneu = np.array([s * np.sin(z) * np.cos(alfa), 
                         ...
                         s * cos(z)])
        dane_wyj.append([dneu[0], dneu[1],dneu[2]])
    return(dane_wyj)

def BL2XY2000(plik_txt,a,e2):
    dane_wej = Odczyt_pliku(plik_txt)
    dane_wyj = []
    for i in dane_wej:
        B,L = i
